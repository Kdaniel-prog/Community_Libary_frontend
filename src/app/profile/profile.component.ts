...
export class ProfileComponent implements OnInit {
  ...
  getBooks() {
    this.httpClient.get<Book[]>('https://localhost:7165/api/book/mybooks?id='+this.OwnerID+'&page='+this.mybookPage+'&size='+this.mybookTableSize,this.httpOptions).subscribe(
      response => {
        this.books = response
        console.log(this.books)
      });
      this.httpClient.get<number>('https://localhost:7165/api/book/mybooksSize?id='+this.OwnerID,this.httpOptions).subscribe(
        response => {
  ...
  getBorrowedBooks() {
    this.httpClient.get<BorrowedBook[]>('https://localhost:7165/api/borrowed/Books?borrowerID='+this.OwnerID+'&page='+this.borrowBooksPage+'&size='+this.borrowBooksTableSize,this.httpOptions).subscribe(
      ...
      this.httpClient.get<number>('https://localhost:7165/api/borrowed/BooksSize?id='+this.OwnerID,this.httpOptions).subscribe(
        response => {
  ...
  userInfo(){
    this.httpClient.get<any>('https://localhost:7165/api/userReviews/oneUser?id='+this.OwnerID,this.httpOptions).subscribe(
      response => {
        console.log(response)
  ...
  onEditBookReview(){
    ...
    getBookReviews(bookid: number) {
      this.bookid = bookid;
      this.httpClient.get<Reviews[]>('https://localhost:7165/api/bookreview/allreview?bookid='+bookid+'&page='+this.page+'&size='+this.tableSize,this.httpOptions).subscribe(
        response => {
          console.log(response)
          this.reviews = response
        });
        this.httpClient.get<number>('https://localhost:7165/api/bookreview/getSize?bookid='+bookid,this.httpOptions).subscribe(
          response => {
            this.count = response
          });
    }
